...
st.write(
    "This is a scatterplot of 3000 scenarios with randomised weights of each asset to determine the optimal portfolio distribution"
)
# ----- LOCAL VARIABLES -----
...
with c1:
    ...
    cal.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(color="crimson", size=12, opacity=1),
    ...
    cal.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(color="orange", size=12, opacity=1),
    ...
    cal.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(color="green", size=12, opacity=1, symbol="star"),
...
with c2:
    ...
    st.write(
        "The :orange[orange] point is the portfolio with the best risk-adjusted return"
    )
    st.write(
        "The small :green[green] star is the portfolio with the most optimal risk-return tradeoff"
    )
...
with tab1:
    ...
    if investors_port[1] * 100 > 0:
        st.write(
            str(round(investors_port[1] * 100, 2))
            + "% of the portfolio, which is $"
            + str(round(investors_port[1] * sum(values), 0))
            + " needs to be allocated to a risk free asset, like a Treasury bill to lower the volatility of the portfolio"
        )
    else:
        ...
        st.write(
            "$"
            + str(diff)
            + " worth of additional investment will help to lower portfolio risk "
        )
    ...
    for i, weight in enumerate(port1):
        with cols[i]:
            st.metric(
                label=tickers[i],
                value="$" + str(round(weight * sum(values), 0)),
